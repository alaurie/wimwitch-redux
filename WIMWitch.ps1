...
.DESCRIPTION 
 ...
 Version 3.0.0
 -Adds Configuration Manger Console Extension to build new image, edit image
 configuration, and to build image
 -Adds free space check
 -Adds command line parameter to bypass free space check
 #>
...
# -inject Start Menu and Default App XMLs
# -import registy files
# -Supports Windows 10 and Windows Server 2016/2019
# -Integrates with ConfigMgr
# -Create ISO files
# -Create OS Upgrade Packages
# -Supports install.esd (ISOs made from Media Creation Tool)
# -Install and use Configuration Manager console extension
#
#
#################################################
#
...
Param( 
   [parameter(mandatory = $false, HelpMessage = "enable auto")] 
    ...
    [ValidateSet("all", "1709", "1803", "1809", "1903", "1909","2004","20H2")] 
        ...
        <TabControl x:Name="TabControl" Margin="-3,-1,3,1" Background="#FFACACAC" BorderBrush="#FF610536" >
            ...
            <TabItem Header="Update Catalog" Height="20" MinWidth="100" Margin="-2,0,-2,0" VerticalAlignment="Top">
                <Grid>
                    ...
                    <CheckBox x:Name="UpdatesW10_1903" Content="1903" HorizontalAlignment="Left" Margin="251,162,0,0" VerticalAlignment="Top" IsEnabled="False"/>
                    ...
                    <CheckBox x:Name="UpdatesW10_1909" Content="1909" HorizontalAlignment="Left" Margin="194,162,0,0" VerticalAlignment="Top" IsEnabled="False"/>
                    ...
                    <CheckBox x:Name="UpdatesW10_2004" Content="2004" HorizontalAlignment="Left" Margin="132,162,0,0" VerticalAlignment="Top" IsEnabled="False"/>
                    ...
                    <CheckBox x:Name="UpdatesW10_20H2" Content="20H2" HorizontalAlignment="Left" Margin="66,162,0,0" VerticalAlignment="Top" IsEnabled="False"/>
                </Grid>
            </TabItem>
            ...
            <TabItem Header="ConfigMgr" Height="20" MinWidth="102">
                <Grid>
                    ...
                    <TextBlock HorizontalAlignment="Left" Margin="445,37,0,0" TextWrapping="Wrap" Text="Click to install CM Console Extension" VerticalAlignment="Top"/>
...
#    }
    ...
    If (($WPFMISCBISO.IsChecked -eq $true) -or ($WPFMISCBUpgradePackage.IsChecked -eq $true)){
        ...
        if ((check-isobinariesexist) -eq $False) {return}
    }
    ...
    if ($WPFUSCBSelectCatalogSource.SelectedItem -eq "OSDSUS"){   
        if ($WPFUpdatesW10Main.IsChecked -eq $true){
            if ($WPFUpdatesW10_20H2.IsChecked -eq $true){
                ...
                download-patches -build 2009 -OS "Windows 10"}
...
#    If ($WPFSourceWimVerTextBox.text -like "10.0.19041.*") { $buildnum = 2004 }
    ...
    If (($WPFSourceWimVerTextBox.text -like "10.0.19041.*") -and (($class -ne "Dynamic") -and ($class -notlike "PE*"))){ 
            $mountdir = $WPFMISMountTextBox.Text
            reg LOAD HKLM\OFFLINE $mountdir\Windows\System32\Config\SOFTWARE | Out-Null
            $regvalues = (Get-ItemProperty -Path "Registry::HKEY_LOCAL_MACHINE\OFFLINE\Microsoft\Windows NT\CurrentVersion\" )
            $buildnum = $regvalues.ReleaseId
    ...
    If (($WPFSourceWimVerTextBox.text -like "10.0.19041.*") -and (($class -eq "Dynamic") -or ($class -like 'PE*'))) {
        $windowsver = Get-WindowsImage -ImagePath ($PSScriptRoot + '\staging\' + $WPFMISWimNameTextBox.text) -Index 1
...
Function Select-Appx {
    ...
    If ($WPFSourceWimVerTextBox.text -like "10.0.19041.*") { 
        ...
        if ($winver -eq 2004){$exappxs = write-output $appx2004 | out-gridview -title "Select apps to remove" -passthru} 
        if ($winver -eq 2009){$exappxs = write-output $appx2009 | out-gridview -title "Select apps to remove" -passthru} 
        }
...
#Adding in optional updates
    ...
    if (($UpdateName -like "* 1903 *") -or ($UpdateName -like "* 1909 *") -or ($UpdateName -like "* 2004 *") -or ($UpdateName -like "* 20H2 *")){$WMIQueryFilter = "LocalizedCategoryInstanceNames = 'Windows 10, version 1903 and later'"}
    ...
    if ($prod -eq 'Windows 10'){     
        ...
        if ($ver -eq "2009"){$ver = "20H2"} #20H2 naming work around
...
try{
    ...
    #$currentuser = (whoami)
    #update-log -Data $currentuser -Class Information
...
function stage-isomedia{
    ...
    try{
        ...
        new-item -path $PSScriptRoot\staging -name "Media" -ItemType Directory -ErrorAction Stop
